...
import { Ean13Barcode } from './products/Ean13Barcode'
...
export function CatalogPageRenderer({ catalog, page, pageNumber, onSelectItem }: { catalog: Catalog; page: CatalogPage; pageNumber: number; onSelectItem?: (id: string) => void }) {
  const isGrid = ['BRAND_PRODUCTS', 'PRODUCTS', 'PRODUCT_GRID_12', 'PRODUCT_GRID_16'].includes(page.pageType)
  const slots = productGridCapacity(page.templateCode, page.pageType)
  ...
  return <article className={clsx('a4-page', page.pageType === 'COVER' && 'a4-cover')}>
    ...
    {isGrid && <div className="print-content"><PrintHeader label="OFERTA PRODUKTOWA" page={pageNumber}/><div className="grid-heading"><div><span className="print-eyebrow">MARKA</span><h2>{page.manufacturerRef || 'Wybrane produkty'}</h2></div>{stringValue(page, 'brandLogo') ? <img className="brand-logo" src={stringValue(page, 'brandLogo')} alt={page.manufacturerRef ?? 'Logo marki'}/> : <div className="grid-heading-mark">LECHTOM<span>.</span></div>}</div><div className={clsx('product-grid', slots === 15 && 'product-grid-15', slots === 16 && 'product-grid-16')}>{Array.from({ length: slots }, (_, index) => { const item = page.items.find((value) => value.slotKey === `product${String(index + 1).padStart(2, '0')}`); const product = item?.product; const image = item?.overrides.customImage || product?.image; return <div className={clsx('product-tile', product && onSelectItem && 'is-editable', !product && !onSelectItem && 'is-empty')} key={index} role={product && onSelectItem ? 'button' : undefined} tabIndex={product && onSelectItem ? 0 : undefined} onClick={() => { if (item && onSelectItem) onSelectItem(item.id) }} onKeyDown={(event) => { if (item && onSelectItem && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); onSelectItem(item.id) } }}>{product ? <>{page.configuration.showEan !== false && !item.overrides.hiddenFields.includes('ean') && <Ean13Barcode ean={product.ean} tall={slots === 15}/>}<div className="product-image"><ProductImage src={image} alt="" className="catalog-product-picture" eager/></div><strong>{item.overrides.displayName || product.name}</strong><div className="product-meta"><span>{page.configuration.showUnit !== false && !item.overrides.hiddenFields.includes('unit') ? item.overrides.weightLabel || product.unit.displayUnit : ''}</span><span>{page.configuration.showBox !== false && !item.overrides.hiddenFields.includes('box') ? item.overrides.boxLabel || (product.itemsPerBox ? `${product.itemsPerBox} szt.` : '') : ''}</span></div>{page.configuration.showCode === true && !item.overrides.hiddenFields.includes('code') && <small className="product-code">{product.code}</small>}</> : <div className="product-placeholder"><span>+</span><small>Slot {String(index + 1).padStart(2, '0')}</small></div>}</div> })}</div><PrintFooter page={pageNumber}/></div>}

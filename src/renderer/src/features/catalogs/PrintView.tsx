...
export function PrintView({ id }: { id: string }) {
  ...
  useEffect(() => { if (!catalog) return; let active = true; void (async () => { await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve))); await document.fonts.ready; await Promise.race([Promise.all(Array.from(document.images).map((img) => img.decode().catch(() => undefined))), new Promise((resolve) => setTimeout(resolve, 9000))]); await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve))); if (active) window.__CATALOG_PRINT_READY__ = true })(); return () => { active = false } }, [catalog])
